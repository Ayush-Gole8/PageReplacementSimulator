document.addEventListener('DOMContentLoaded', function() {
  ...
  function simulateFIFO(pages, frameCount) {
    ...
    for (let i = 0; i < pages.length; i++) {
      const page = pages[i];
      const isPageFault = !queue.includes(page);
      
      // Create a copy of current frame state before any changes
      const currentFrames = [...queue];
      while (currentFrames.length < frameCount) {
      ...
      result.push({
        frames: isPageFault ? 
          [...queue, ...Array(frameCount - queue.length).fill(null)] : 
          currentFrames,
        currentPage: page,
  ...
  function simulateLRU(pages, frameCount) {
    ...
    for (let i = 0; i < pages.length; i++) {
      const page = pages[i];
      const isPageFault = !stack.includes(page);
      
      // Create a copy of current frame state before any changes
      const currentFrames = [...stack];
      while (currentFrames.length < frameCount) {
        currentFrames.push(null);
      }
      
      if (stack.includes(page)) {
        // If page is already in memory, move it to the most recently used position
        const idx = stack.indexOf(page);
        stack.splice(idx, 1);
      } else {
        // If page is not in memory and memory is full, remove least recently used page
      ...
      result.push({
        frames: isPageFault ? 
          [...stack, ...Array(frameCount - stack.length).fill(null)] : 
          currentFrames,
        currentPage: page,
        pageFault: isPageFault
      });
  ...
  function simulateOptimal(pages, frameCount) {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (isPageFault) {
        if (memory.length < frameCount) {
          ...
          const nextUse = memory.map(p => {
            const nextIndex = future.indexOf(p);
      ...
      result.push({
        frames: isPageFault ? 
          [...memory, ...Array(frameCount - memory.length).fill(null)] : 
          currentFrames,
        currentPage: page,
        pageFault: isPageFault
      });
